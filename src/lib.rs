...
pub fn process(file: &str, newcrc: &str, offset: &str, printstatus: bool) -> Option<String> {
    // Handle arguments
    let offset = match u64::from_str(offset) {
        Ok(x) => x,
    ...
    let newcrc = match u32::from_str_radix(newcrc, 16) {
        Ok(x) => x.reverse_bits(),
    ...
    if !file.is_file() {
        return Some(format!("Error: File does not exist: {:?}", file));
    }

    // Process the file
    match modify_file_crc32(file, offset, newcrc, printstatus) {
        Ok(_) => None,
        Err(e) => {
...
fn modify_file_crc32(
    ...
    let mut raf = std::fs::OpenOptions::new()
        .read(true)
        .write(true)
        .open(file)?;
    if length < 4 || offset > length - 4 {
        return Err(io::Error::new(
            ...
            "Byte offset plus 4 exceeds file length",
        ));
    }
    ...
    assert_eq!(
        get_crc32(&mut raf)?,
        newcrc,
        "Failed to update CRC-32 to desired value"
    );
    if printstatus {
